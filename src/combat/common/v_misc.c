...
void vrx_pvm_spawn_monsters(edict_t* self, int max_monsters, int total_monsters)
{
	...
	while (total_monsters < max_monsters && max_spawn_this_cycle > 0) {
		...
		edict_t* scan;
		if ((scan = vrx_create_new_drone(self, rnd, true, true, self->monsterinfo.level)) != NULL) {
...
void FindMonsterSpot(edict_t* self) {
	...
	float msg_threshold = ffa_respawntime->value * 0.5f;

	if (level.time > self->delay) {
		int total_monsters = PVM_TotalMonsters(self, true);
		...
		if (total_monsters < max_monsters) {
			if (total_monsters < threshold) {
				...
				if (total_monsters <= levelup_threshold && self->last_move_time < level.time && self->count) {
					gi.bprintf(PRINT_HIGH, "Monsters are trying to push back and have leveled up!\n");
					self->monsterinfo.level += max(1, self->monsterinfo.level);
					self->last_move_time = level.time + 15.0;
				}

				float last_time = self->random;
				self->random *= 0.9;

				if (self->random < msg_threshold && last_time > msg_threshold)
				{
					gi.bprintf(PRINT_HIGH, "Monsters are trying to send forces more often!\n");
				}
				...
				if (self->random > msg_threshold && last_time < msg_threshold)
				{
					gi.bprintf(PRINT_HIGH, "Monsters are retreating their forces.\n");
				}


				if (self->last_move_time < level.time && self->monsterinfo.level > 0) {
					self->monsterinfo.level *= 0.75f;
					self->last_move_time = level.time + 15.0;

					if (self->monsterinfo.level > 0)
						gi.bprintf(PRINT_HIGH, "Monsters are getting cocky and are sending weaker forces.\n");
					else
						gi.bprintf(PRINT_HIGH, "Monsters are back to normal levels.\n");
				}
		...
		WriteServerMsg(va("World has %d/%d monsters. Next update in %.1f seconds. lupt: %d", total_monsters, max_monsters,
